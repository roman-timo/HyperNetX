...
def get_lhs(params, n, cj, d, c):
    ...
    if (n, cj, d, c) not in LHS_CACHE:
        LHS_CACHE[(n, cj, d, c)] = params.q[d] * params.wcd[sum_, d] * comb(d-sum_, c-sum_) * (cj/n)**(c-sum_) * (1-cj/n)**(d-c)
    ...
    for i in sorted(range(len(params.w)), key=lambda k: params.w[k], reverse=True):
        ...
        if loc == -1:
            ...
            for j in range(len(slots)):
                ...
                wts = slots[comm_idxs]
                if sum(wts) == 0:
                    warnings.warn("hypergraph is too tight. Failed to find community for node $i")
...
def config_model(clusters, params, he1):
    ...
    edges_with_missing_size = list[tuple[[], int]]() # partial edges with missing d-c slots
    ...
    for j in range(len(params.s)):
        ...
        for d in range(L-1, 0, -1):
            ...
            if sumq2 > 0:
                new_md = [ range(d+1, L)[i] * md[d+1:L][i] for i in range(len(md[d+1:L]))]
        ...
        while pj > sumpj:
            dec_idx = random.choices(range(len(cluster_idxs)), k=1, weights=[params.y[i] for i in cluster_idxs])[0]
            params.y[dec_idx] -= 1
            params.z[dec_idx] += 1
            pj -= 1
        ...
        if pj == 0:
            ...
            for d in range(1, L):
                for c in range(d, (d-1) // 2, -1):
                    ...
                    if sumwfd > 0:
                        mcd[c, d] = randround(params.wcd[c, d] / sumwfd * (md[d] - sum(mcd[c:d+1, d])))
            ...
            if tail_size > 0:
                ...
                additional_points = np.random.choice(range(len(yc_rem)), tail_size, p=normalized_weights, replace=False)
            ...
            for d in range(1, L):
                for c in range((d+1) // 2, d+1):
                    for _ in range(mcd[c, d]):
                        ...
                        edges_with_missing_size.append((internal_stumps[stump_idx:stump_end+1], d-c))
                        stump_idx += c
            ...
            y_cluster_idxs_less_yc_base = [y_cluster_idxs[i] - yc_base[i] for i in range(len(y_cluster_idxs))]
    ...
    if len(background_stumps):
        print(f"moved {len(background_stumps)} non-matched stumps from community graphs to background graph")
    ...
    if stump_idx+1 <= len(background_stumps):
        ...
        if params.q[0] > 0 and (not params.is_simple or (len(left_stumps) == len(np.unique(left_stumps))) and
                         len(np.intersect1d([he1_[0] for he1_ in he1], left_stumps)) == 0):
            ...
            assert to_add > 0
            extra_stumps = random.choices(range(len(params.w)), weights=params.w, k=to_add)
            left_stumps.append(*extra_stumps)
            assert len(left_stumps) == targetq
            edges.append(sorted(left_stumps))
            print(f"added degree to the following nodes due to parity issues: {extra_stumps}")
    ...
    if params.is_simple:
        ...
        if bad_multi > 0:
            ...
            print(f"fixing {bad_multi} hyperedges ({bad_multi_pct}% of total number of hyperedges) that were multisets")
        if bad_dup > 0:
            bad_dup_pct = round(100 * bad_dup / len(edges), ndigits=2)
            print(f"fixing {bad_dup} hyperedges ({bad_dup_pct}% of total number of hyperedges) that were duplicated")

        random.shuffle(bad_edges)

        iters = params.maxiter * len(bad_edges)
        for _ in range(iters):
            if not len(bad_edges): break

            bad_edge = bad_edges.pop()
            initial_badness = (tuple(bad_edge) in good_edges) + (len(bad_edge) - len(np.unique(bad_edge)))
            if initial_badness == 0:
                ...
                final_bandess = (new1 in list(good_edges)) + (len(new1) - len(np.unique(new1))) + \
                                (new2 in list(good_edges)) + (len(new2) - len(np.unique(new2)))
                if final_bandess < initial_badness:
                    if len(new1) == len(np.unique(new1)) and tuple(new1) not in list(good_edges):
                        good_edges.add(tuple(new1))
                    else:
                        bad_edges.append(new1)

                    if len(new2) == len(np.unique(new2)) and tuple(new2) not in list(good_edges):
        ...
        if len(bad_edges):
            print(f"""Failed to fix all bad edges in {params.maxiter} rounds.
...
def sample_communities(t2, c_min, c_max, n, max_iter):
    ...
    print(f"failed to sample an admissible community sequence in {max_iter} draws. Fixing")
    ...
    while best_ss != n:
        ...
        if change > 0:
            if not best_s[i] < c_max: continue
        else:
            if not best_s[i] > c_min: continue
...
def abcdh_hypergraph(n, dss, css, x, q, ws, seed, m=False, stats=False, prefix=None):
    ...
    if n != sum(coms):
        warnings.warn(f"number of vertices {n} does not match the sum of community sizes {sum(coms)}")
...
if __name__ == "__main__":
    he, cl = abcdh_hypergraph(1000, [2.5,5,10], [1.5,10,100], 0.5, [0.0,0.4,0.3,0.2,0.1], ":linear", 1234)
    print("hyperedges\n", he, "\n\n")
    print("clusters\n", cl, "\n\n")
